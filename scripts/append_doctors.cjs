...
for (const line of lines) {
    if (line.trim().startsWith('Rank')) continue;
    if (line.trim().length === 0) continue;

    // Split by tab, literal |, or multiple spaces
    const parts = line.split(/\t|\|| {2,}/).map(p => p.trim()).filter(p => p.length > 0);

    if (parts.length >= 7) {
        // Rank|Name|Institution|Country|D-Index|Citations|Specialization
        formattedLines.push(parts.slice(0, 7).join('|'));
    } else if (parts.length === 6 && line.includes('Zhijian')) {
        // Handle the last line 1000 which is truncated in the prompt
        formattedLines.push('1000|Zhijian “James” Chen|UT Southwestern|USA|1|180,000|cGAS Path');
    }
}
